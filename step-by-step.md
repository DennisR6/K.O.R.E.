...
| 53 | `[-]` | `milestone-53-official-item-gameplay-qualification` | — | — | Official Item Gameplay Qualification | Active blocker confirmed by testers: items are not working and the item picker is confusing. Reconcile the item source implementation with the `effect-disappears-after-use` qualification findings, fix the picker flow and feedback, then verify every official item visibly changes real matches. Qualify targeting, stacking, expiration, inventory limits, snapshots, replays, reconnects, UI feedback, and item balance for all supported economies. |
