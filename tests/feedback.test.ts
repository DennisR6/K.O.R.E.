...
test.serial("feedback is accumulated in the database and online submissions require membership", async () => {
	...
	const second = await serveFeedback(new Request("http://localhost/api/feedback", { method: "POST", body: JSON.stringify({ mode: "hotseat", mapId: "ice-map-v1", text: "Needs more maps", topic: "balance", rating: 3 }) }), database);
	...
	expect(database.listFeedback().find(entry => entry.text === "Needs more maps")).toMatchObject({ topic: "balance", rating: 3 });
	const invalidTopic = await serveFeedback(new Request("http://localhost/api/feedback", { method: "POST", body: JSON.stringify({ mode: "hotseat", text: "bad", topic: "cheating" }) }), database);
