...
| R-08 | TODO Phase 6; GDD 7 and 12 | [Data Contracts](step-by-step.md#4-data-contracts-mods-and-maps) | `tests/map_document.test.ts` covers canonical map physics, geometry, spawns, hazards, and metadata; `tests/map_loader.test.ts` covers hazard-free conversion; `tests/hazard_registry.test.ts` covers hazard registry validation; `tests/setting_effect.test.ts` verifies map-configured death circles only. | Hazard runtime/effect tests, validated Cue Clash/Frostbite/Magma maps, and editor-export conversion tests. |
