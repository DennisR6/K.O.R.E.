...
- The SDK main menu offers local, online, AI, map, Mods, and Human Playtest
  actions. Human Playtest opens a short instruction screen and launches the
  canonical local match; completed-match records and performance are reported
  through the existing offline-match pipeline, while the normal feedback prompt
  collects the player's observations.
...
- `src/server/feedbackRoute.ts` and `src/net/feedback.ts`: validated native
  ...
  online matches also automatically submit one `topic: bug` feedback entry per
  stale or hash-mismatched network packet with the game context, turn, hashes,
  and diagnostic data; `GameDatabase` accumulates the entries in
  `user_feedback` for later review.
