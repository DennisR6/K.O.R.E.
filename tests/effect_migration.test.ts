...
test("the migration boundary upgrades historical unversioned Effects recursively", () => {
	...
	expect(migrated).toEqual({ schemaVersion: 1, type: EffectType.Multi, typeValue: [{ schemaVersion: 1, type: EffectType.Damage, typeValue: { damage: 5 } }] });
...
test("full Effect migration preserves trigger composition without moving trigger logic into Effects", () => {
	...
	expect(migrated).toEqual({ schemaVersion: 1, trigger: EffectTrigger.Collision, triggerValue: [], type: EffectType.Damage, typeValue: { damage: 5 } });
