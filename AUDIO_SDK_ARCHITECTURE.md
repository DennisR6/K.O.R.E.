...
| Main-menu callbacks | `src/kore/ui/KoreMainMenuSurface.ts` | `ISoundEmitter`, command queue | Menu music and confirm mapping | Applies aggregate batch | The SDK-authored main menu is the production UI composition; immediate scene changes are relayed by `LocalMatchSceneRouter`. |
