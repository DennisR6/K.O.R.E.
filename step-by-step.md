...
| 59 | `[x]` | `milestone-59-gameplay-animation-polish` | — | — | Gameplay Animation Polish | Completed deterministic presentation for shot trails, charge/power, collision/hazard/damage/elimination bursts, shield rings, item pickup/use markers, turn transitions, AI-thinking feedback, and result celebration. All effects remain outside authoritative snapshots and use the semantic feedback/presentation boundary; focused tests, TypeScript validation, and browser build pass. |
| 60 | `[-]` | `milestone-60-database-feedback-action-plan` | — | [`database-feedback-action-list.md`](database-feedback-action-list.md) | Database Feedback Action Plan | Analyzed the downloaded SQLite telemetry export and audited each finding against current source/tests. Several items are implemented or partially implemented; browser proof, desync recovery, item timing clarity, mobile qualification, balance, and telemetry-quality work remain. |
| 61 | `[-]` | `milestone-61-ranked-mode-readiness` | — | [`ranked-mode-readiness.md`](ranked-mode-readiness.md) | Ranked Mode Readiness | Ranked is not yet enabled. The readiness plan defines the required authenticated identity, rating ledger, ranked queue, frozen ruleset/map rotation, authoritative result finalization, disconnect/abuse policy, leaderboard, audit tools, tests, and release gates. |
