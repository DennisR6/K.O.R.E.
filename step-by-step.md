...
| 51 | `[ ]` | `milestone-51-core-playable-release-blockers` | — | — | Core Playable Release Blockers | Complete the external two-match human playtest, fix all blocker-level usability defects, and make objective, active team, phase, selected actor, aim, power, item state, playback, result, rematch, and menu flows understandable without intervention. Completion requires updated playtest evidence with no unresolved blocker. |
| 52 | `[ ]` | `milestone-52-ai-match-termination-and-agency` | — | — | AI Match Termination And Agency | Eliminate hard-AI safety-limit matches and repeated no-op behavior; make AI evaluate hazards, out-of-bounds risk, self-elimination, positioning, and meaningful tactical progress. The 1,000-match RC fuzz run, 5,000-match soak run, fairness tournament, and supported map/mode matrix must terminate within defined limits. |
| 53 | `[ ]` | `milestone-53-official-item-gameplay-qualification` | — | — | Official Item Gameplay Qualification | Reconcile the item source implementation with the `effect-disappears-after-use` qualification findings, then verify every official item visibly changes real matches. Qualify targeting, stacking, expiration, inventory limits, snapshots, replays, reconnects, UI feedback, and item balance for all supported economies. |
| 54 | `[ ]` | `milestone-54-final-map-set-qualification` | — | — | Final Map Set Qualification | Decide the release map roster, fix or remove blocked `frostbite-arena`, and resolve documented map-specific AI, drift, fairness, duration, and instant-death issues. Every selectable map must pass technical, browser, replay, restore, termination, and human playtest qualification. |
| 55 | `[ ]` | `milestone-55-gameplay-balance-and-content-tuning` | — | — | Gameplay Balance And Content Tuning | Establish target match lengths and balance friction, drift, spawns, obstacles, hazards, figure counts, item economy, and AI difficulty. Remove excessive first-turn advantage, endless matches, unavoidable losses, and strategies that are not understandable or meaningful to human players. |
| 56 | `[ ]` | `milestone-56-production-ux-and-input-platforms` | — | — | Production UX And Input Platforms | Finish tutorial/onboarding, action rejection feedback, loading/connection states, audio feedback, accessibility, responsive camera behavior, controller mappings, touch controls, Steam Deck layout, and mobile-device verification. Completion requires clean-build tests on every platform claimed for release. |
