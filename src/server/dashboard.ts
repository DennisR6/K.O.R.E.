...
export async function serveDashboard(request: Request, registry: Pick<GameRegistry, "getMetrics" | "killGame">, config: DashboardConfig, database?: Pick<GameDatabase, "exportSnapshot" | "listOperatorReplays" | "getOperatorReplay" | "createOperatorReplayView" | "getDashboardPerformanceMetrics" | "listFeedback">, publicBaseUrl?: string): Promise<Response | undefined> {
	...
	try {
		...
		return new Response(renderDashboard(body, publicBaseUrl, database?.listFeedback() ?? []), {
...
function renderDashboard(metrics: DashboardMetricsResponse, publicBaseUrl?: string, feedback: StoredFeedback[] = []): string {
	...
	const feedbackRows = feedback.map(entry => `<article class="rounded-xl border border-slate-200 bg-slate-50 p-4"><div class="flex flex-col justify-between gap-2 sm:flex-row sm:items-start"><div class="flex flex-wrap items-center gap-2"><span class="rounded-full bg-cyan-100 px-2.5 py-1 text-xs font-semibold text-cyan-700">${escapeHtml(entry.mode ?? "unknown")}</span>${entry.mapId ? `<span class="text-xs font-medium text-slate-500">${escapeHtml(entry.mapId)}</span>` : ""}${entry.gameId ? `<span class="font-mono text-xs text-slate-400">${escapeHtml(entry.gameId)}</span>` : ""}</div><time class="text-xs text-slate-400" datetime="${new Date(entry.createdAt).toISOString()}">${escapeHtml(new Date(entry.createdAt).toLocaleString("en-US"))}</time></div><p class="mt-3 whitespace-pre-wrap text-sm leading-6 text-slate-700">${escapeHtml(entry.text)}</p>${entry.userId ? `<p class="mt-2 text-xs text-slate-400">Player: ${escapeHtml(entry.userId)}</p>` : ""}</article>`).join("") || `<p class="text-sm text-slate-400">No feedback yet.</p>`;
	return `<!doctype html>
