...
function buildUi(language: LanguageCatalog): UiMenuSettings {
	...
	builder.addScreen(ui.screen({ id: KoreHudScreen.Main, layout: ui.layout.absolute(), elements: [
		...
		ui.text({ id: KoreHudElement.ItemsTitle, text: translate(language, KoreHudText.Items), rect: rect(18, 365, 560, 22), style: KoreHudStyle.ItemsTitle, visible: false }),
		...KORE_HUD_ITEM_SLOTS.map(slot => ui.button({ id: koreHudItemElementId(slot), text: "", rect: rect(18 + slot * 54, 395, 46, 46), style: KoreHudStyle.Item, enabled: false, visible: false })),
		ui.button({ id: KoreHudElement.SkipItem, text: translate(language, KoreHudText.SkipItemPhase), rect: rect(620, 398, 165, 38), style: KoreHudStyle.Skip, action: ui.action.emit(KoreHudCommand.SkipItemPhase), visible: false }),
