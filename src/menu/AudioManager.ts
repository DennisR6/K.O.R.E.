...
export class AudioManager {
	...
	private readonly tracks = Object.values(KORE_AUDIO_ASSETS).slice(0, 10);
	...
	private playPlaylistTrack(): void { const url = this.tracks[this.currentTrackIndex]; if (!url) return; this.applyCommand({ type: "playMusic", sourceId: "playlist", soundId: Object.keys(KORE_AUDIO_ASSETS)[this.currentTrackIndex] ?? "kore.music.menu", bus: "music", priority: 10, runtimeId: "browser", globalSourceId: "browser:playlist", sequence: 0, replacementPolicy: "replace-current" }); }
}
