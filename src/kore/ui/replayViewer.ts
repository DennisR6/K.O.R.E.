...
export function createReplayViewerComposition(): KoreReplayViewerSettings {
	const settings: KoreReplayViewerSettings = {
		...
		ui: ui.createMenu({ id: "kore.replay-viewer.ui", size: { width: 800, height: 450 } })
			.addScreen(ui.screen({
				...
				elements: [
					ui.text({ id: KoreReplayElement.Title, text: "Replay viewer", rect: { x: 0, y: 0, width: 480, height: 32 }, style: "kore.replay.title" }),
					ui.textInput({ id: KoreReplayElement.Token, text: "", value: "", rect: { x: 0, y: 0, width: 480, height: 34 }, style: "kore.replay.token" }),
					ui.button({ id: KoreReplayElement.Load, text: "Load replay", rect: { x: 0, y: 0, width: 180, height: 36 }, style: "kore.button.blue", action: ui.action.emitValues(KoreReplayCommand.Load, [KoreReplayElement.Token]) }),
					ui.button({ id: KoreReplayElement.Paste, text: "Paste from clipboard", rect: { x: 0, y: 0, width: 220, height: 36 }, style: "kore.button.blue-back", action: ui.action.emit(KoreReplayCommand.Paste) }),
