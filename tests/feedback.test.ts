...
test("feedback endpoint preserves deployment paths and posts feedback", async () => {
	...
	expect(ok).toBe(true);
	expect(JSON.parse(await request!.text())).toEqual({ mode: "hotseat", mapId: "ice-map-v1", text: "Fun match" });
});
...
test.serial("feedback is accumulated in the database and online submissions require membership", async () => {
	...
	const second = await serveFeedback(new Request("http://localhost/api/feedback", { method: "POST", body: JSON.stringify({ mode: "hotseat", mapId: "ice-map-v1", text: "Needs more maps" }) }), database);
