...
| R-05 | TODO Phase 4; GDD 4 and 8 | [Baseline and Architecture](step-by-step.md#1-baseline-and-architecture) and [Match Rules](step-by-step.md#3-match-rules-and-round-lifecycle) | `tests/rule_types.test.ts` covers serializable rule contracts; `tests/turn_system.test.ts` covers turn progression; `tests/authoritative_game.test.ts` covers authoritative turn ownership. | Item/shot/physics rule transition tests, match-result snapshots, input lock, local rematch, and authoritative rematch tests. |
