...
test("frozen replay records validate, persist independently, and let both participants export one stable share", () => {
	...
	const secondExport = db.createReplayShare("game", replay, 3);
	expect(secondExport.token).toBe(share.token);
	expect(secondExport.replay.actions).toEqual([]);
	db.close();
});
