...
const damageDefinition = { schemaVersion: 1 as const, id: "trap.explode", effect: { type: EffectType.Damage, typeValue: { damage: 5 } } };
...
test("TriggerDefinitionCatalog rejects duplicates, unknown IDs, invalid Effects, and executable fields", () => {
	...
	expect(() => validateTriggerDefinition({ ...damageDefinition, effect: { type: EffectType.Damage, typeValue: { damage: -1 } } })).toThrow();
...
test("TriggerDefinition supports MultiEffect declaration order", () => {
	const catalog = new TriggerDefinitionCatalog().register({ schemaVersion: 1, id: "combo", effect: { type: EffectType.Multi, typeValue: [damageDefinition.effect] } });
