...
test("counter mutations live in canonical state and survive snapshot restore", () => {
	...
	runtime.applyCounterEffect({ schemaVersion: 1, type: COUNTER_SET_EFFECT_ID, target, typeValue: { value: 5 } });
	runtime.applyCounterEffect({ schemaVersion: 1, type: COUNTER_ADD_EFFECT_ID, target, typeValue: { amount: -2 } });
	...
	restored.applyCounterEffect({ schemaVersion: 1, type: COUNTER_RESET_EFFECT_ID, target, typeValue: {} });
...
test("counter mutations reject unknown targets deterministically", () => {
	const handler = createCanonicalPlayableMatchHandler();
	handler.addSystem(new CounterSystem());
	expect(() => handler.applyCounterEffect({ schemaVersion: 1, type: COUNTER_ADD_EFFECT_ID, target: { type: "counter", counterId: "missing" }, typeValue: { amount: 1 } })).toThrow("Unknown counter target");
});
