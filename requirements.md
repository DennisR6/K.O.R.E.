...
| R-02 | TODO Phase 2 figure state and shot mechanics; GDD 5 | [Physics](step-by-step.md#2-deterministic-physics-and-arena-safety) | `tests/ui_input.test.ts` covers drag angle/power; `tests/physics_test.ts` covers vector math and circle collision; `tests/deterministic_turn.test.ts` snapshots a real-map turn. | Player rotation/angular state round trips; shared forward-vector impulse tests; force-modifier tests. |
