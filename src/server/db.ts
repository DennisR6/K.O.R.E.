...
export class GameDatabase {
	...
	public storeFeedback(feedback: FeedbackSubmission, now: number = Date.now()): StoredFeedback {
		const id = crypto.randomUUID();
		this.db.query("INSERT INTO user_feedback (id, game_id, user_id, mode, map_id, text, created_at) VALUES (?1, ?2, ?3, ?4, ?5, ?6, ?7)")
			.run(id, feedback.gameId ?? null, feedback.userId ?? null, feedback.mode ?? null, feedback.mapId ?? null, feedback.text.trim(), now);
	...
	public listFeedback(limit = 100): StoredFeedback[] {
		...
		const rows = this.db.query("SELECT id, game_id, user_id, mode, map_id, text, created_at FROM user_feedback ORDER BY created_at DESC, id ASC LIMIT ?1").all(bounded) as Array<{ id: string; game_id: string | null; user_id: string | null; mode: FeedbackSubmission["mode"] | null; map_id: string | null; text: string; created_at: number }>;
		return rows.map(row => ({ id: row.id, ...(row.game_id ? { gameId: row.game_id } : {}), ...(row.user_id ? { userId: row.user_id } : {}), ...(row.mode ? { mode: row.mode } : {}), ...(row.map_id ? { mapId: row.map_id } : {}), text: row.text, createdAt: row.created_at }));
	}
