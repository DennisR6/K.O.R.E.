...
class PresentationRuntime {
  ...
  record(event, sequence) {
    return { ...clone(event), sequence, tick: this.tickNumber };
  }
  frame(events) {
    return { schemaVersion: 1, runtimeId: this.runtimeId, tick: this.tickNumber, events: events.map(clone), animations: [...this.active.values()].sort(byInstance).map((item) => this.projectAnimation(item)) };
  }
  ...
  restoreActive(item) {
    ...
    this.active.set(item.channel, clone(item));
  }
}
...
var presentation = {
  createAnimation(settings) {
    const result = { schemaVersion: 1, ...clone(settings) };
  ...
  play(eventId, animationId, options = {}) {
    return { schemaVersion: 1, type: "play", eventId, animationId, ...clone(options) };
  },
  cancel(eventId, options) {
    return { schemaVersion: 1, type: "cancel", eventId, ...clone(options) };
  },
