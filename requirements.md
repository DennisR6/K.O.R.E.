...
| R-05 | TODO Phase 4; GDD 4 and 8 | [Baseline and Architecture](step-by-step.md#1-baseline-and-architecture) and [Match Rules](step-by-step.md#3-match-rules-and-round-lifecycle) | `tests/rule_types.test.ts` covers serializable rule contracts; `tests/rule_interpreter.test.ts` covers configured phases and item allowance; `tests/engine_emitter.test.ts` covers local rule-driven progression; `tests/turn_system.test.ts` and `tests/team_figure_counts.test.ts` cover configured player/figure layouts; `tests/authoritative_game.test.ts` covers rule-driven authoritative turn ownership. | Shot/physics rule transition tests, match-result snapshots, input lock, local rematch, and authoritative rematch tests. |
