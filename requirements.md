...
| R-05 | TODO Phase 4; GDD 4 and 8 | [Baseline and Architecture](step-by-step.md#1-baseline-and-architecture) and [Match Rules](step-by-step.md#3-match-rules-and-round-lifecycle) | `tests/rule_types.test.ts` covers serializable rule contracts; `tests/rule_interpreter.test.ts` covers item allowance and required staged-shot transitions; `tests/ui_input.test.ts` covers simulation, playback, and match-end input locks; `tests/rule_state_snapshot.test.ts` covers persisted rule progress and match results; `tests/winning_system.test.ts` covers configurable last-team-standing evaluation; `tests/engine_emitter.test.ts` covers local rule-driven progression; `tests/turn_system.test.ts` and `tests/team_figure_counts.test.ts` cover configured player/figure layouts; `tests/authoritative_game.test.ts` covers rule-phase protocol synchronization and authoritative turn ownership. | Local rematch and authoritative rematch tests. |
